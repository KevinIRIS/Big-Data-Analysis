...
if __name__ == "__main__":
    ...
    def repack(list):
        ...
        return res
    dataset.select([count(when(column(index) == "invalid", index)).alias(index) for index in dataset.columns]).rdd.map(lambda x: repack(x)).coalesce(1).saveAsTextFile("count_invalid_col.csv")
    sc.stop()
