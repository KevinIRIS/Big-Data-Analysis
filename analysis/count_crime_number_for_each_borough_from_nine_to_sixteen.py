...
if __name__ == "__main__":
    ...
    res = res.filter(res["year_data"] >= 2009).filter(res["yaer_data"] <= 2016)
    for borough in boroughs:
        df = dataframe.filter(column("BORO_NM") == borough).groupBy("year").count()
        res = df.join(res, df["year"] == res["year_data"]).drop(df.year)
    res.save('count_crime_number_for_each_borough_from_nine_to_sixteen.csv', 'com.databricks.spark.csv')
    sc.stop()